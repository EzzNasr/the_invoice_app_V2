...
async def get_dashboard_stats(db: sqlite3.Connection = Depends(get_db)):
    ...
    c.execute("""
        SELECT od.Item_ID, SUM(od.Quantity) as total_qty
        FROM OrderDetails od
        GROUP BY od.Item_ID
        ORDER BY total_qty DESC
        LIMIT 5
    """)
    top_items = [{"name": r[0], "qty": r[1]} for r in c.fetchall()]
...
async def generate_invoice(invoice_req: InvoiceRequest, db: sqlite3.Connection = Depends(get_db)):
    ...
    # in QuantityInput.tsx so no need to re-branch here).
    ...
    if invoice_req.bill_type == "actual":
        real_invoice_number = functions.Process2_pure(db, pipeline_data)
